...
#include "vm.h"
#include "memory.h"
#define UINT8_COUNT (UINT8_MAX + 1) //ekstremno sus
typedef struct {
  Token current;
  Token previous;
  bool hadError;
...
static void subscript(bool canAssign) {
  ...
  consume(TOKEN_R_BRACKET, "Expect ']' after index.");
...
static void list(bool canAssign) {
    ...
    consume(TOKEN_R_BRACKET, "Expect ']' after list literal.");
...
ParseRule rules[] = {
  ...
  [TOKEN_L_BRACKET]     = {list,  subscript, PREC_SUBSCRIPT},
...
static void classDeclaration() {
  ...
  if (match(TOKEN_LESS) || match(TOKEN_COLON)) {
